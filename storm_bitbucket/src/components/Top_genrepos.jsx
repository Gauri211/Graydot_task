...
const Top_genrepos = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          'https://spit-hackthn.vercel.app/top-ml-repos/?language=R&topic=image-processing&top_n=4',
          {
            headers: {
  ...
  return (
    <div>
      <h1 style={{ textAlign: 'center', marginBottom: '20px' }}>Top Image Processing Repositories</h1>
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
        {repos.map((repo) => (
          ...
          >
            ...
            <h3 style={{ marginBottom: '10px' }}>{repo.name}</h3>
